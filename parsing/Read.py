...
from enums import HeaderTypes
from enums import TimeSeriesHeaderTypes as TimeSeriesHeaderTypes
...
def _write_to_well(well_manager:WellManager, api, lists):
    ...
    for i in lists.items():
        w.add_or_append_series(i[0], lists[TimeSeriesHeaderTypes.date], i[1])#Currently storing the dates redundantly for each series, this may be optimizable with some loss of generality
...
def parse_time_series(config:BaseConfig, well_manager:WellManager, data_directory:str):
    ...
    try:
        header_to_column = _get_column_indices(header_line, config.headers_to_read + [TimeSeriesHeaderTypes.api])
    except KeyError:
    ...
    for l in all_lines:
        ...
        if api != current_api:#Write in large batches to minimize resizing of underlying numpy array
            ...
            current_api = api
        try:
            date = datetime.strptime(sp[header_to_column[TimeSeriesHeaderTypes.date]], '%Y-%m-%d %H:%M:%S').timestamp() #Convert to seconds since linux epoch. Ignoring timezones
            oil_barrels = float(sp[header_to_column[TimeSeriesHeaderTypes.oil_barrels]])
            water_barrels = float(sp[header_to_column[TimeSeriesHeaderTypes.water_barrels]])
            gas_mcf = float(sp[header_to_column[TimeSeriesHeaderTypes.gas_mcf]])
        except ValueError:
            continue
